...
public class SpecialEditC_PaymentCurrencyRate implements ISpecialEditCallout {
	...
	public String validateEdit(GridTab mTab, GridField mField, PO po, Object newValue) {
		...
		BigDecimal CurrencyRateEntered = (BigDecimal) newValue;
		
		if (newValue == null) {
			Message = "*** "+Msg.translate(Env.getCtx(),"invalid")+" **** "+
					" ("+Msg.translate(Env.getCtx(),"CurrencyRate")+"="+CurrencyRateEntered+")";
	...
	public boolean updateEdit(GridTab mTab, GridField mField, PO po, Object newValue) {
		...
		pay.setCurrencyRate((BigDecimal)newValue);
		BigDecimal newCurrencyRate = pay.getCurrencyRate();
		BigDecimal newConvertedAmt = pay.getPayAmt().multiply(newCurrencyRate);
		pay.setConvertedAmt(newConvertedAmt);
		pay.saveEx();
